...
const ModalMoveFile = ({ isOpen, onClose, file }) => {
  const [currFolder, setCurrFolder] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const [queryMessage, setQueryMessage] = useState("");
  const context = useOutletContext();
  const { mutateFiles, mutateFolders, folders, setNav } = context;
  ...
  const onMoveFolder = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SERVER}/api/files/move`,
        {
          method: "PUT",
          credentials: "include",
          ...
          <ModalBody className="p-4 text-xl">
            ...
            <div
              className={`pl-3 pt-2 text-sm ${
                queryMessage == "File move successful"
                  ? "text-green-500"
                  : "text-red-500"
              }`}
            >
